...
if (class_exists('WP_Customize_control')) {
    ...
    class Online_Portfolio_Repeater_Controler extends WP_Customize_Control
    {
        /**
         * The control type.
         *
        ...
        public $online_portfolio_box_label = '';

        public $online_portfolio_box_add_control = '';
        ...
        {
            $this->fields = $fields;
            $this->online_portfolio_box_label = $args['online_portfolio_box_label'];
            $this->online_portfolio_box_add_control = $args['online_portfolio_box_add_control'];
        ...
        <?php } ?>
            ...
            <ul class="online_portfolio-repeater-field-control-wrap">
                <?php
                $this->online_portfolio_get_fields();
                ?>
            </ul>

            <input type="hidden" <?php $this->link(); ?> class="online_portfolio-repeater-collector"
                   value="<?php echo esc_attr($this->value()); ?>"/>
            <button type="button"
                    class="button online_portfolio-add-control-field"><?php echo esc_html($this->online_portfolio_box_add_control); ?></button>
            <?php
        }

        private function online_portfolio_get_fields()
        {
            $fields = $this->fields;
            $values = json_decode($this->value());
            ...
            if (is_array($values)) {
                foreach ($values as $value) {
                    ?>
                    <li class="online_portfolio-repeater-field-control">
                        <h3 class="online_portfolio-repeater-field-title"><?php echo esc_html($this->online_portfolio_box_label); ?></h3>

                        <div class="online_portfolio-repeater-fields">
                            <?php
                            foreach ($fields as $key => $field) {
                                $class = isset($field['class']) ? $field['class'] : '';
                                ?>
                                <div
                                    class="online_portfolio-fields online_portfolio-type-<?php echo esc_attr($field['type']) . ' ' . $class ?>">
                                    ...
                                    switch ($field['type']) {
                                        ...
                                        case 'upload':
                                            ...
                                            echo '<div class="online_portfolio-fields-wrap">';
                                            echo '<div class="attachment-media-view">';
                                            echo '<div class="placeholder' . $image_class . '">';
                                            esc_html__('No image selected', 'online-portfolio');
                                            echo '</div>';
                                            echo '<div class="thumbnail thumbnail-image">';
                                            echo $image;
                                            echo '</div>';
                                            echo '<div class="actions clearfix">';
                                            echo '<button type="button" class="button online_portfolio-delete-button align-left">' . esc_html__('Remove', 'online-portfolio') . '</button>';
                                            echo '<button type="button" class="button online_portfolio-upload-button alignright">' . esc_html__('Select Image', 'online-portfolio') . '</button>';
                                        ...
                                        case 'category':
                                            ...
                                            echo '<option value="0">' . esc_html__('Select Category', 'online-portfolio') . '</option>';
                                            echo '<option value="-1">' . esc_html__('Latest Posts', 'online-portfolio') . '</option>';
                                        ...
                                        case 'colorpicker':
                                            echo '<input data-default="' . esc_attr($default) . '" class="online_portfolio-color-picker" data-alpha="true" data-name="' . esc_attr($key) . '" type="text" value="' . esc_attr($new_value) . '"/>';
                                            break;
                                        ...
                                        case 'range':
                                            ...
                                            echo '<div class="online_portfolio-range-slider" >';
                                        ...
                                        case 'icon':
                                            echo '<div class="online_portfolio-selected-icon">';
                                            echo '<i class="' . esc_attr($new_value) . '"></i>';
                                            echo '<span><i class="fa fa-angle-down"></i></span>';
                                            echo '</div>';
                                            echo '<ul class="online_portfolio-icon-list clearfix">';
                                            $online_portfolio_font_awesome_icon_array = online_portfolio_font_awesome_icon_array();
                                            foreach ($online_portfolio_font_awesome_icon_array as $online_portfolio_font_awesome_icon) {
                                                $icon_class = $new_value == $online_portfolio_font_awesome_icon ? 'icon-active' : '';
                                                echo '<li class=' . $icon_class . '><i class="' . $online_portfolio_font_awesome_icon . '"></i></li>';
                                            }
                                            echo '</ul>';
                                            echo '<input data-default="' . esc_attr($default) . '" type="hidden" value="' . esc_attr($new_value) . '" data-name="' . esc_attr($key) . '"/>';
                                            break;
                                        ...
                                        case 'multicategory':
                                            ...
                                            echo '<ul class="online_portfolio-multi-category-list">';
                                            echo '<li><label><input type="checkbox" value="-1" ' . checked('-1', $new_value, false) . '/>' . esc_html_e('Latest Posts', 'online-portfolio') . '</label></li>';
                                            foreach ($this->cats as $cat) {
                            ...
                            <div class="clearfix online_portfolio-repeater-footer">
                                <div class="alignright">
                                    <a class="online_portfolio-repeater-field-remove"
                                       href="#remove"><?php esc_html_e('Delete', 'online-portfolio') ?></a> |
                                    <a class="online_portfolio-repeater-field-close"
                                       href="#close"><?php esc_html_e('Close', 'online-portfolio') ?></a>
                                </div>
                            </div>
                        </div>
                    </li>
...
if (!function_exists('online_portfolio_font_awesome_icon_array')) {
    function online_portfolio_font_awesome_icon_array()
    {
